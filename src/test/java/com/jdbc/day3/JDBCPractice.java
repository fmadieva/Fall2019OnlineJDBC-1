...
//here we are throwing the runtime exception
//because we want our code to compile and we use try catch, but we also want to throw exception
//to show there is exception, so throw it as runtime exception here.
//if we only use e.printstrack, it will show the exception on the console
//but it will not fail the test. The test still will pass. So, by throwing runtime exception here
// if something happens the test will fail.